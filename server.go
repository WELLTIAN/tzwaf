...
import (
	...
	"github.com/WELLTIAN/tzwaf/routers"
...
func main() {
	...
	m.Group("/admin", func() {
		...
		m.Group("/rule/", func() {
			...
			m.Get("/list/:type", routers.ListRules)
			m.Get("/new/:type", routers.NewRule)
			m.Post("/new/:type", csrf.Validate, routers.DoNewRule)
			m.Get("/edit/:id/:type", routers.EditRule)
			m.Post("/edit/:id/:type", csrf.Validate, routers.DoEditRule)
			m.Get("/del/:id/:type", routers.DelRule)
	   ...
	   m.Group("/dashboard/", func() {
		...
		m.Group("/log/", func() {
			...
			m.Get("/list/", routers.ListLogsApi)
			m.Get("/list/top", routers.ListLogTopApi)
			m.Get("/list/total", routers.ListLogTotalApi)
    	})
})
	m.Group("/api", func() {
	...
	log.Printf("TZwaf admin %s", setting.AppVer)
